...
import genericTags from "@/assets/db/tags_genericas.json";
...
const categoryIcons: Record<string, (color: string) => React.ReactNode> = {
  "Associação": (color) => <FontAwesome5 name="handshake" size={28} color={color} />,
  "Supermercado": (color) => <MaterialIcons name="shopping-cart" size={28} color={color} />,
  "Saúde": (color) => <FontAwesome5 name="hospital" size={28} color={color} />,
  "Farmácia": (color) => <FontAwesome5 name="capsules" size={28} color={color} />,
  "Loja": (color) => <MaterialIcons name="store" size={28} color={color} />,
  "Shopping": (color) => <MaterialIcons name="local-mall" size={28} color={color} />,
  "Escola": (color) => <Ionicons name="school" size={28} color={color} />,
  "Serviço": (color) => <MaterialIcons name="build" size={28} color={color} />,
  "Banco": (color) => <FontAwesome5 name="university" size={28} color={color} />,
  "Órgão Público": (color) => <Ionicons name="business" size={28} color={color} />,
  "Cafeteria": (color) => <FontAwesome5 name="coffee" size={28} color={color} />,
  "Eletrônicos": (color) => <MaterialIcons name="devices" size={28} color={color} />,
  "Transporte": (color) => <MaterialIcons name="directions-bus" size={28} color={color} />,
};
...
const formatCityName = (city: string) =>
  city.toLowerCase().split(" ").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
...
export default function App() {
  ...
  const uniqueCities = useMemo(() =>
    Array.from(new Map(locations.map((loc) => [loc.cidade.toLowerCase(), loc.cidade])).values()).sort()
  , []);

  const uniqueCategories = useMemo(() => Array.from(new Set(Object.values(genericTags))).sort(), []);
  ...
  const cityCenters = useMemo(() => {
    ...
    return Object.fromEntries(
      Object.entries(acc).map(([city, { sumLat, sumLon, count }]) => [city, { latitude: sumLat / count, longitude: sumLon / count }])
    );
  }, []);
  ...
  const loadUserLocation = async () => {
    ...
    } catch (err) { console.error("Erro ao carregar localização:", err); }
  };
  ...
  const onRefresh = async () => { setRefreshing(true); await loadUserLocation(); setRefreshing(false); };

  const filteredLocations = useMemo(() => {
    if (!selectedCity) return [];
    return locations
      .filter(loc => loc.cidade.toLowerCase() === selectedCity.toLowerCase())
      .filter(loc => loc.nome_estabelecimento.toLowerCase().includes(searchText.toLowerCase()) ||
                     loc.endereco.toLowerCase().includes(searchText.toLowerCase()))
      .filter(loc => selectedFilters.length === 0 || selectedFilters.includes(genericTags[loc.tag]))
      .map(loc => {
        let distKm = userLocation ? getDistanceKm(userLocation.latitude, userLocation.longitude, loc.latitude, loc.longitude) : Number.MAX_SAFE_INTEGER;
        return { ...loc, distance: distKm !== Number.MAX_SAFE_INTEGER ? `${distKm.toFixed(1)} Km` : "N/A", distanceKm: distKm };
      })
      .sort((a, b) => a.distanceKm - b.distanceKm);
  }, [selectedCity, userLocation, searchText, selectedFilters]);

  const renderItem = ({ item }: { item: LocationItem & { distance: string } }) => (
    <TouchableOpacity onPress={() => openMaps(item.latitude, item.longitude, item.nome_estabelecimento)} style={{ marginBottom: 10 }}>
    ...
    <SafeAreaView style={styles.safe}>
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.header}>
          <RoundButton style={styles.searchButton} onPress={() => setSearchVisible(!isSearchVisible)}>
          ...
          <View style={{ flex: 1 }}>
            <RoundButton style={styles.locationButton} onPress={() => setCityListVisible(!isCityListVisible)}>
              <Text style={{ color: "#fff", fontWeight: "bold" }}>{selectedCity ? formatCityName(selectedCity) : "Carregando..."}</Text>
            </RoundButton>
            ...
            {isCityListVisible && (
              <View style={styles.cityListContainer}>
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <TouchableOpacity style={styles.cityItem} onPress={() => { setSelectedCity(item); setCityListVisible(false); }}>
          ...
          <RoundButton style={styles.filterButton} onPress={() => setFilterVisible(true)}>
            ...
            <View style={styles.modalContent}>
              ...
              <FlatList
                ...
                renderItem={({ item }) => {
                  ...
                  return (
                    <TouchableOpacity style={[styles.filterSquare, isSelected && styles.filterSquareSelected]} onPress={() => toggleFilter(item)}>
                      <View style={{ marginBottom: 6 }}>
                        {categoryIcons[item] ? categoryIcons[item](color) : <Ionicons name="help" size={28} color={color} />}
                      </View>
                      <Text style={[styles.filterSquareText, isSelected && { color: "#fff" }]} numberOfLines={1}>{item}</Text>
                    </TouchableOpacity>
                  );
                }}
              />
...
const styles = StyleSheet.create({
  ...
  searchButton: { backgroundColor: "#EEEFEF", borderColor: "#BBBDC0", borderWidth: 1, paddingHorizontal: 8, paddingVertical: 8, borderRadius: 999, marginLeft: 6, marginRight: 6 },
  ...
  searchInput: { flex: 1, borderWidth: 1, borderColor: "#ccc", borderRadius: 8, paddingHorizontal: 10, height: 40, marginRight: 6 },
  ...
  filterSquare: { flex: 0, width: '30%', aspectRatio: 1, margin: 6, borderRadius: 12, backgroundColor: "#f2f2f2", justifyContent: "center", alignItems: "center", padding: 8 },
  filterSquareSelected: { backgroundColor: "#000" },
  filterSquareText: { fontSize: 12, textAlign: "center", color: "#000", fontWeight: "500" },
  closeButton: { marginTop: 12, backgroundColor: "#000", paddingVertical: 12, borderRadius: 8, alignItems: "center" },
